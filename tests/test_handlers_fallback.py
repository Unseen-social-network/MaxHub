...
async def test_looks_like_command_matches_slash_prefixed_text():
    filt = LooksLikeCommand()

    assert await filt(_make_event(1, "/xyz")) is True
    assert await filt(_make_event(1, "  /xyz with args")) is True


async def test_looks_like_command_rejects_plain_text():
    filt = LooksLikeCommand()

    assert await filt(_make_event(1, "просто сообщение в чат")) is False
