...
6. **Mini App** (`/app`) — веб-версия части механик: список дел (чтение и добавление), слово дня с подпиской, рассылка для админов. Открывается кнопкой прямо в чате, работает на отдельном пути `MINIAPP_PATH` того же домена.
